...
public class SoftmaxPolicy implements DifferentiablePolicy {
	protected DifferentiableQPlanner planner;
	protected double temperature;
	protected double[] policy;
	protected double[][] log_grad_policy;
	
	public SoftmaxPolicy(DifferentiableQPlanner planner, double temperature){
	...
	public OutputAndGradient2D evaluate(State st) {
		...
		/*    Softmax policy: mu(a) = exp(tau*Q(a)) / (sum_b exp(tau*Q(b)))
	    ...
	    for(int a = 0; a < num_actions; a++) {	      
	      this.policy[a] = Math.exp(temperature * (Q[a] - max));
	      denom += this.policy[a];
	    }        
	     ...
	     *       dmu(a)/dtheta = tau * mu(a) * (dQ(a) - sum_b (mu(b) * dQ(b)))
	     */
	    ...
	    for (int i = 0; i < num_reward_features; ++i) {
	    	...
	    	for (int a = 0; a < num_actions; ++a) 
	    		this.log_grad_policy[a][i] = temperature * policy[a] * (grad_Q[a][i] - sum);
	    }
